...
System::load_sys_class('blockPay');
...
class cart extends base {
	...
	*/
	...
	public function getqrcode(){
		...
		$rake = blockPay::getMoneyByRake($result['money'],$currency);

		$pay_checkbox=isset($_POST['moneycheckbox_b']) ? true : false;
		// 商品参数
		$cartlist=(json_decode(stripslashes($this->Cartlist),true));
		//$score_used = isset($_POST['shop_score_b']) ? $_POST['shop_score_b'] : 0;
		//$score_num = isset($_POST['shop_score_num_b']) ? $_POST['shop_score_num_b'] : 0;

		//处理充值的价格  start
		if(isset($_POST['shop_score_b'])){
			$fufen_cfg = System::load_app_config("user_fufen",'','member');	
			$fufen = intval($_POST['shop_score_num_b']);			
			if($fufen_cfg['fufen_yuan']){				
				$fufen = intval($fufen / $fufen_cfg['fufen_yuan']);
				$fufen = $fufen * $fufen_cfg['fufen_yuan'];
			}
		...
		$currency = $_POST['type'];
		//商品数量
		$shoplen = $_POST['shoplen'];
		if(!$currency || !$moneyCount || !$shoplen){
			_message("参数错误！");
		}
		$rake = blockPay::getMoneyByRake($currency);
		if(!$rake){
			_message("参数错误！");
		}
		$uid = intval(_encrypt(_getcookie("uid"), 'DECODE'));
		...
		$result = $pay->go_pay_block($pay_checkbox);

		/*  2016-1-8 
			$result 的返回值有两种情况
			1 为 true  时  直接支付不需要充值  【积分兑换后的金额 >= 应付金额】 
			2 为 array 时  需要充值  		   【积分兑换后的金额 <  应付金额】 
		*/
		if(is_array($result)){
			var_dump($result);
			$recordinfo = _encrypt_pay(json_encode($result));
			//根据金额获取兑换后的相关币种的支付
			$blockMoney = $result['money'];
			if(!$blockMoney){
				//汇率转换失败
				$return['msg'] = '发生错误，请稍后再试或者换其他支付方式！';
				echo json_encode($return);
				exit;
			}
			//订单编号
			$dingdancode = $result['dingdancode'];
			//收款人邮箱
			$payee = System::load_sys_config('pay','payee');
			//创建二维码中的数据
			$arr = array(
					'sdata' => $result,
					//'cdata' => array('currency' => $currency,'payee' => $payee,'blockMoney' => $blockMoney)
				);
			$str0 = _encrypt_pay(json_encode($arr));
			//二维码处理
			$str1 = 'assets_,_'.$currency.'_,_'.$payee.'_,_'.$blockMoney.'_,_'.$str0.'_,_walet';
			$str = _encrypt_pay(json_encode($str1)); //加密
			$qrcode = create_qr_code_block($str,$uid,$currency.$shoplen); //创建二维码，返回二维码图片链接

			//二维码图片
			$qrpic = get_home_url().'/'.strstr($qrcode,'statics');

			include templates("cart","qrcode");	
	...
	//检查商品是否已经支付成功 
	public function checkbcrecord(){
		$sinfo = $this->segment(4);
		$info_json = _encrypt_pay($sinfo,'DECODE');
		if(empty($info_json)){
			_message("参数错误!",WEB_PATH.'/member/home/userbuylist');
		}
		$info = json_decode($info_json,true);
		if(!is_array($info)){
			_message("参数错误!",WEB_PATH.'/member/home/userbuylist');
		}
		$code = $info['dingdancode'];
		$money = $info['money'];
		$dingdaninfo = $this->db->GetOne("select * from `@#_member_addmoney_record` where `code` = '$code' and `money` = '$money' and `status` = '已付款' for update");
		if($dingdaninfo){
			//成功
			header("location: ".WEB_PATH."/member/cart/paysuccess");
		}else{
			_message("系统检测到您没有支付成功!",WEB_PATH.'/member/cart/cartlist');
		}
	}
}
